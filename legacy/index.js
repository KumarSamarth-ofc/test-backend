...
app.get("/health", (req, res) => {
  res.json({
    success: true,
    message: "Stoory Backend is running (legacy)",
...
app.get("/test-socket", (req, res) => {
  ...
  if (ioInstance) {
    ioInstance.emit("test_message", testMessage);
  }
...
app.post("/test-message", async (req, res) => {
  try {
    ...
    ioInstance.to(`room:${conversationId}`).emit("new_message", {
      conversation_id: conversationId,
      message: testMessage,
    ...
    ioInstance.to(`user_${senderId}`).emit("message_sent", {
      conversation_id: conversationId,
      message: testMessage,
      conversation_context: conversationContext,
    });
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`🚀 Stoory Backend (legacy) server running on port ${PORT}`);
